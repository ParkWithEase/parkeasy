...
type carR struct {
	UseruuidUser *User // car.car_useruuid_fkey
}
...
type carJoins[Q dialect.Joinable] struct {
	typ          string
	UseruuidUser func(context.Context) modAs[Q, userColumns]
}
...
func buildCarJoins[Q dialect.Joinable](cols carColumns, typ string) carJoins[Q] {
	return carJoins[Q]{
		typ:          typ,
		UseruuidUser: carsJoinUseruuidUser[Q](cols, typ),
	}
}
...
func carsJoinUseruuidUser[Q dialect.Joinable](from carColumns, typ string) func(context.Context) modAs[Q, userColumns] {
...
// UseruuidUser starts a query for related objects on users
func (o *Car) UseruuidUser(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) UsersQuery {
	return Users.Query(ctx, exec, append(mods,
		sm.Where(UserColumns.Useruuid.EQ(psql.Arg(o.Useruuid))),
	)...)
}

func (os CarSlice) UseruuidUser(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) UsersQuery {
	...
	return Users.Query(ctx, exec, append(mods,
		sm.Where(psql.Group(UserColumns.Useruuid).In(PKArgs...)),
	)...)
}
...
func PreloadCarUseruuidUser(opts ...psql.PreloadOption) psql.Preloader {
...
func ThenLoadCarUseruuidUser(queryMods ...bob.Mod[*dialect.SelectQuery]) psql.Loader {
	return psql.Loader(func(ctx context.Context, exec bob.Executor, retrieved any) error {
		loader, isLoader := retrieved.(interface {
			LoadCarUseruuidUser(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
		})
		if !isLoader {
			return fmt.Errorf("object %T cannot load CarUseruuidUser", retrieved)
		}

		err := loader.LoadCarUseruuidUser(ctx, exec, queryMods...)
...
// LoadCarUseruuidUser loads the car's UseruuidUser into the .R struct
func (o *Car) LoadCarUseruuidUser(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}

	// Reset the relationship
	o.R.UseruuidUser = nil

	related, err := o.UseruuidUser(ctx, exec, mods...).One()
	if err != nil {
		return err
	}

	related.R.UseruuidCar = o

	o.R.UseruuidUser = related
	return nil
}

// LoadCarUseruuidUser loads the car's UseruuidUser into the .R struct
func (os CarSlice) LoadCarUseruuidUser(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}

	users, err := os.UseruuidUser(ctx, exec, mods...).All()
	if err != nil {
		return err
	}
...
func attachCarUseruuidUser0(ctx context.Context, exec bob.Executor, count int, car0 *Car, user1 *User) (*Car, error) {
	setter := &CarSetter{
		Useruuid: omit.From(user1.Useruuid),
	}

	err := Cars.Update(ctx, exec, setter, car0)
	if err != nil {
		return nil, fmt.Errorf("attachCarUseruuidUser0: %w", err)
	}

	return car0, nil
}

func (car0 *Car) InsertUseruuidUser(ctx context.Context, exec bob.Executor, related *UserSetter) error {
	user1, err := Users.Insert(ctx, exec, related)
	if err != nil {
		return fmt.Errorf("inserting related objects: %w", err)
	}

	_, err = attachCarUseruuidUser0(ctx, exec, 1, car0, user1)
	if err != nil {
		return err
	}

	car0.R.UseruuidUser = user1

	user1.R.UseruuidCar = car0

	return nil
}

func (car0 *Car) AttachUseruuidUser(ctx context.Context, exec bob.Executor, user1 *User) error {
	var err error
